...
module.exports = async function handler(req, res) {
    ...
    if (req.method === "GET") {
        ...
        return res.status(200).json({
            ...
            activeContract: activeContract && {
                ...
                seasonsCompleted: activeContract.seasons_completed || 0
            },
    ...
    if (req.method === "PUT") {
        const { player, advanceSeasonContractId } =
            req.body || {};
        ...
        let seasonsCompleted;

        if (advanceSeasonContractId) {
            seasonsCompleted = await advanceContractSeason(
                db,
                advanceSeasonContractId
            );
        }

        return res.status(200).json({ ok: true, seasonsCompleted });
    }
