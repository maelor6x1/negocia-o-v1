...
async function enterAuthenticatedApp() {
    ...
    careerData = await apiRequest("/api/career");

    trustMap = careerData.trust || {};

    if (
        careerData.player &&
        Object.keys(careerData.player).length > 0
    ) {
        player = careerData.player;
    }
...
async function finalizeAcceptedContract(goals) {
    if (pendingRenewal) {
        ...
        adjustTrustLocal(currentBrand.name, 3);
    }

    await saveAcceptedContract(
        currentBrand.name,
        currentOffer,
        goals
    );
...
Bem-vindo à ${currentBrand.name}.`
    ...
    finalizeAcceptedContract(goals);
}
...
// Preenchimento vertical usado ao abrir o card (mesmo valor em qualquer
// tamanho de tela, controlado por JS em vez de CSS/media query).
const RIVAL_CARD_PADDING_Y = 20;
    ...
    typingQueue = typingQueue.then(
        () =>
            new Promise((resolve) => {
                ...
                // Começa fechado: sem altura, sem opacidade, sem
                // padding vertical — tudo controlado via estilo
                // inline pra não depender de nenhuma técnica de CSS.
                rivalMessage.style.maxHeight = "0px";
                rivalMessage.style.opacity = "0";
                rivalMessage.style.paddingTop = "0px";
                rivalMessage.style.paddingBottom = "0px";
                rivalMessage.style.overflow = "hidden";

                rivalMessage.innerHTML = `
                    <div class="rivalInner">
                        <div class="rivalHeader">📩 PROPOSTA DE OUTRA MARCA</div>
                        ...
                        <p class="rivalOfferPreview">
                            Oferecem € ${formatMoney(rivalOfferValue)} por semana
                            caso você feche com eles.
                        </p>
                ...
                requestAnimationFrame(() => {
                    // Mede a altura real do conteúdo com o padding já
                    // "aberto" aplicado, mas antes de animar — scrollHeight
                    // inclui esse padding mesmo com overflow:hidden.
                    rivalMessage.style.paddingTop =
                        RIVAL_CARD_PADDING_Y + "px";

                    rivalMessage.style.paddingBottom =
                        RIVAL_CARD_PADDING_Y + "px";

                    const targetHeight =
                        rivalMessage.scrollHeight;

                    rivalMessage.style.transition =
                        "max-height 0.5s cubic-bezier(.65, 0, .35, 1), opacity 0.4s ease";

                    requestAnimationFrame(() => {
                        rivalMessage.style.maxHeight =
                            targetHeight + "px";
...
function disableRivalButtons() {
    document
        .querySelectorAll(".rivalActions button")
        .forEach((button) => {
            button.disabled = true;
        });
}

// Fecha o card rival animando de volta pra altura zero (usa a mesma
// transição inline definida na abertura, então roda suave nos dois
// sentidos sem depender de nenhuma classe CSS especial).
function closeRivalCard(rivalMessageEl) {
    if (!rivalMessageEl) {
        return;
    }

    // Garante que existe uma altura definida antes de fechar, caso o
    // card ainda esteja no meio da animação de abertura.
    rivalMessageEl.style.maxHeight =
        rivalMessageEl.scrollHeight + "px";

    requestAnimationFrame(() => {
        rivalMessageEl.style.maxHeight = "0px";
        rivalMessageEl.style.opacity = "0";
        rivalMessageEl.style.paddingTop = "0px";
        rivalMessageEl.style.paddingBottom = "0px";
    });
}
...
function acceptRivalOffer(button) {
    ...
    if (rivalMessageEl) {
        // Flash verde de confirmação, seguido do mesmo recolhimento
        // usado ao ignorar — mantém o mesmo "idioma visual".
