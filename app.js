    ...
    if (!contract) {
        ...
        activeCard.innerHTML = `
            ...
            <div class="dealInfo careerDealInfo">
                ...
                <div>
                    <span>DURAÇÃO</span>
                    <strong>${contract.duration} temporada(s)</strong>
                </div>
    ...
    typingQueue = typingQueue.then(
        () =>
            new Promise((resolve) => {
                ...
                // Insere fechado e abre no frame seguinte, pra o
                // navegador animar a transição em vez de já nascer aberto.
                requestAnimationFrame(() => {
                    requestAnimationFrame(() => {
                        rivalMessage.classList.add("rivalOpen");
...
function dismissRivalOffer(button) {
    ...
    if (rivalMessageEl) {
        // Remover "rivalOpen" recolhe o card (grid-template-rows volta
        // a 0fr) — o mesmo movimento da abertura, só que ao contrário.
        rivalMessageEl.classList.add("rivalClosing");
        rivalMessageEl.classList.remove("rivalOpen");
